<div class="box box-primary">
	<div class="box-body">
		...
		<a href="<?= base_url('C_home/deleteAllUjian9') ?>" onclick="return confirm('Apa anda yakin ?')" class="btn btn-danger"><i class="fa fa-trash-o"></i> Hapus Semua</a>
		<br /><br />
		<table class="table table-bordered table-hover" id="tablePertama">
			...
			<tbody>
				<?php $no=1; foreach ($dataUjian9 as $row): ?>
					<tr>
					<td><?= $no ?></td>
						<td><?= $row->ks_nama ?></td>
						<td><?= $row->ks_nis ?></td>
						...
						<td><?= $row->ks_walas ?></td>
						<td>
							<a href="<?= base_url('C_home/edit_ujian9/'.$row->id_ujian) ?>"><i class="fa fa-edit"></i></a>
							<a href="<?= base_url('graduated/GraduatedCtrl/ujian9/'.$row->id_ujian) ?>" onclick="return confirm('Are you sure ?')"><i class="fa fa-trash-o"></i></a>
						</td>
					</tr>
				<?php $no++; endforeach ?>
			</tbody>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="row">
					...
					<div class="col-sm-6">
						<form action="<?= base_url('C_home/import_ujian9') ?>" method="POST" enctype="multipart/form-data">
